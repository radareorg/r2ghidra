...
public:
	...
	ghidra::ConstructState *getState() {
		return point;
	}
	...
	int nextOpTpl(ghidra::OpTpl *(&lastop)) {
		...
		if (op->getOpcode() != ghidra::OpCode::CPUI_MULTIEQUAL) { // if NOT a build directive
			lastop = op;
			return 0; // return ordinary OpTpl
		}
		// Get the operand index from the build directive
		curind = (int)op->getIn(0)->getOffset().getReal();
		return curind + 1;
	}
};
...
public:
	R2Sleigh(ghidra::LoadImage *ld, ghidra::ContextDatabase *c_db): R2loader(ld), Sleigh(ld, c_db) {}
	~R2Sleigh() { clearCache(); }
	
	void reset(ghidra::LoadImage *ld,ghidra::ContextDatabase *c_db) { R2loader = ld; Sleigh::reset(ld, c_db); }
...
private:
	...
	struct FlowRecord {
		ghidra::ConstructState *addressnode = nullptr; // Constructor state containing destination address of flow
