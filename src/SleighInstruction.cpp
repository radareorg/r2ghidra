// SPDX-FileCopyrightText: 2020-2023 FXTi, pancake
...
void R2Sleigh::reconstructContext(ParserContext &protoContext) {
	...
	while (walker.isState()) {
		...
		if (ct) {
			...
			if (oper == 0) {
				// Upon first entry to this Constructor
...
void SleighInstructionPrototype::addExplicitFlow(ConstructState *state, OpTpl *op, FlowFlags flags, FlowSummary &summary) {
...
void SleighInstructionPrototype::gatherFlows(std::vector<Address> &res, SleighInstruction *inst, int secnum) {
	std::vector<FlowRecord *> curlist;
	if (secnum < 0) {
		curlist = flowStateList;
	} else if ((!flowStateListNamed.empty()) && (secnum < flowStateListNamed.size())) {
