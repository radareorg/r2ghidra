...
static ut64 read_be64u(const ut8 *p) {
	return ((ut64)p[0] << 56) | ((ut64)p[1] << 48) | ((ut64)p[2] << 40) | ((ut64)p[3] << 32) |
		((ut64)p[4] << 24) | ((ut64)p[5] << 16) | ((ut64)p[6] << 8) | (ut64)p[7];
}

static st64 read_be64s(const ut8 *p) {
	return (st64)read_be64u (p);
}

static ut32 read_be32u(const ut8 *p) {
	return ((ut32)p[0] << 24) | ((ut32)p[1] << 16) | ((ut32)p[2] << 8) | (ut32)p[3];
}

static st32 read_be32s(const ut8 *p) {
	return (st32)read_be32u (p);
}

static ut16 read_be16u(const ut8 *p) {
	return ((ut16)p[0] << 8) | (ut16)p[1];
}
...
static bool read_field_binary(const ut8 *record, int record_len, int *off, ut8 **out, int *out_len) {
	...
	*off += 4;
	...
	ut8 *b = malloc ((size_t)len);
	if (!b) {
		return false;
	}
	memcpy (b, record + *off, (size_t)len);
	*out = b;
	*out_len = len;
	*off += len;
	return true;
...
static bool walk_long_node(FidFile *ff, int node_id, const ut8 *types, int type_count, int fixed_len, RecordCb cb, void *user) {
	...
	case NODE_LONGKEY_FIXED_REC:
		for (int i = 0; i < key_count; i++) {
			...
			if (off + 8 + fixed_len > ff->buffer_size) {
				return false;
			}
			ut64 key = read_be64u (node + off);
...
static bool parse_master_record(FidFile *ff, ut64 key, const ut8 *record, int record_len, void *user) {
	...
	off += 4;
	t.max_key = read_be64s (record + off);
	off += 8;
...
static bool parse_function_record(FidFile *ff, ut64 key, const ut8 *record, int record_len, void *user) {
	...
	int off = 0;
	f.key = key;
	f.code_unit_size = read_be16u (record + off);
	off += 2;
	f.full_hash = read_be64u (record + off);
	off += 8;
	f.specific_hash_additional_size = record[off++];
	f.specific_hash = read_be64u (record + off);
	off += 8;
	f.library_id = read_be64u (record + off);
	off += 8;
	f.name_id = read_be64u (record + off);
	off += 8;
	f.entry_point = read_be64u (record + off);
	off += 8;
	f.domain_path_id = read_be64u (record + off);
	off += 8;
	f.flags = record[off];
...
static bool count_record(FidFile *ff, ut64 key, const ut8 *record, int record_len, void *user) {
	/* unused: ff */ (void)ff;
	/* unused: key */ (void)key;
	/* unused: record */ (void)record;
	/* unused: record_len */ (void)record_len;
	int *count = (int *)user;
	(*count)++;
	return true;
}
...
static bool fid_load_file(const char *path, FidFile *ff) {
	...
	ut64 magic = read_be64u (buf);
	int header_version = read_be32s (buf + 16);
	int block_size = read_be32s (buf + 20);
	if (magic != GHIDRA_GBF_MAGIC || header_version != 1 || block_size <= 5 || (sz % (size_t)block_size)) {
		free (buf);
		return false;
	}
	ff->buf = buf;
...
static bool signature_type_text_is_simple(const char *s) {
	return !strstr (s, "::") && !strstr (s, "<") && !strstr (s, ">") &&
		!strstr (s, "`") && !strstr (s, "__int64") && !strstr (s, "enum ") &&
		!strstr (s, "__m128") && !strstr (s, "__m256") &&
		!strstr (s, "(__cdecl *)") && !strstr (s, "(__stdcall *)") &&
		!strstr (s, "(__fastcall *)") && !strstr (s, "(__thiscall *)") &&
		!strstr (s, ")const") && !strstr (s, ") const");
}
...
static char *trim_dup(const char *start, const char *end) {
	...
	return dup_range0 ((const ut8 *)start, (int)(end - start));
}
